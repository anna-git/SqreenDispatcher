...
namespace SqreenDispatcher
{
    public class AuthorizeSqreenAttribute : Attribute, IAuthorizationFilter
    {
        ...
        public async void OnAuthorization(AuthorizationFilterContext context)
        {
            ...
            var signatureExists = context.HttpContext.Request.Headers.TryGetValue(sqreenOptions.HeaderName, out Microsoft.Extensions.Primitives.StringValues keyv);
            context.HttpContext.Request.EnableBuffering();
            
            if (signatureExists && keyv.Any())
            {
                var result = await Verifier.VerifySignature(sqreenOptions!.SecretKey, context.HttpContext.Request.Body, keyv[0]);
